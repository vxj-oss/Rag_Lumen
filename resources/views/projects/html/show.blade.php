<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header :title="$project->nombre" :subtitle="$project->codigo . ' · ' . ($project->client?->nombre ?? __('Sin cliente'))">
            <x-slot name="actions">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-ui.card padding="p-6 space-y-4">
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.badge :color="$project->prioridad->color()">{{ __('Prioridad') }}: {{ $project->prioridad->label() }}</x-ui.badge>
                    <x-ui.badge :color="$risk['level']->color()">{{ __('Riesgo') }}: {{ $risk['level']->label() }} ({{ $risk['score'] }})</x-ui.badge>
                </div>

                @if ($project->descripcion)
                <p class="text-sm text-gray-700">{{ $project->descripcion }}</p>
                @endif
            </x-ui.card>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <x-ui.card padding="p-6 space-y-4">
                        ...
                        <div class="flex items-center justify-between text-xs pt-2 border-t border-gray-100">
                            <span class="text-gray-500">{{ __('Ajuste por prioridad') }} ({{ $project->prioridad->label() }})</span>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <x-ui.card padding="p-6 space-y-4">
                        <h3 class="font-semibold text-gray-800">{{ __('Información general') }}</h3>
                        <dl class="space-y-3 text-sm">
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Tipo') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->tipo->label() }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Cliente') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->client?->nombre ?? '—' }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Gerente') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->manager?->fullName() ?? '—' }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Líder') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->responsibleEmployee?->fullName() ?? '—' }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3 pt-3 border-t border-gray-100">
                                <dt class="text-gray-500">{{ __('Presupuesto') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->presupuesto ? number_format((float) $project->presupuesto, 2) : '—' }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3 pt-3 border-t border-gray-100">
                                <dt class="text-gray-500">{{ __('Fecha de inicio') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->fecha_inicio->format('d/m/Y') }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Fecha fin estimada') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->fecha_fin_estimada->format('d/m/Y') }}</dd>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <dt class="text-gray-500">{{ __('Fecha fin real') }}</dt>
                                <dd class="text-gray-900 text-right">{{ $project->fecha_fin_real?->format('d/m/Y') ?? '—' }}</dd>
                            </div>
                        </dl>
                        ...
                        <div class="pt-3 border-t border-gray-100">
                            ...
                            <dd class="text-sm text-gray-900 mt-1">{{ $project->observaciones }}</dd>
                        </div>
                        @endif
                    </x-ui.card>
                    ...
                    <x-ui.card padding="p-6 space-y-4">
                        ...
                        <div>
                            <div class="flex items-center justify-between text-sm mb-1">
                                <span class="font-medium text-gray-800">{{ $row['area']->nombre }}</span>
                                <span class="text-gray-500 text-xs">{{ $row['completed'] }}/{{ $row['total'] }} · {{ $row['percent'] }}%</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="bg-emerald-500 h-2 rounded-full" @style(["width: {$row['percent']}%"])></div>
                            </div>
                            <p class="text-xs text-gray-400 mt-0.5">{{ __('ponderado') }} {{ $row['weighted'] }}%</p>
                        </div>
                        @empty
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500">{{ $member->pivot->rol_en_proyecto }}</td>
                            <td class="px-4 py-2 text-sm text-gray-500">{{ $member->pivot->asignado_en->format('d/m/Y') }}</td>
                            <td class="px-4 py-2 text-sm">
                                <x-ui.badge :color="$member->pivot->estado === 'active' ? 'green' : 'gray'">
                                    {{ $member->pivot->estado === 'active' ? __('Activo') : __('Inactivo') }}
                                </x-ui.badge>
                            </td>
                            <td class="px-4 py-2 text-right">
                                @can('update', $project)
                                @if ($member->pivot->estado === 'active')
    ...
    @can('update', $project)
    <x-ui.form-modal name="edit-project" :title="__('Editar proyecto')" :subtitle="$project->codigo"
