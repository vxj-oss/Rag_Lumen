@php
    ...
    if (auth()->user()->hasRole([\App\Support\Enums\RoleName::Administrator->value, \App\Support\Enums\RoleName::Manager->value])) {
...
<div x-data="{ mobileOpen: false }">
    <div class="lg:hidden flex items-center justify-between bg-slate-900 text-white px-4 py-3">
        ...
        <button @click="mobileOpen = true" class="p-2 -mr-2">
    ...
    <aside
        ...
        class="fixed inset-y-0 left-0 z-50 w-64 h-screen bg-slate-900 text-slate-200 flex flex-col transition-transform duration-200 lg:translate-x-0 lg:static lg:sticky lg:top-0 lg:z-auto"
    >
        <div class="h-16 flex items-center gap-2 px-5 border-b border-slate-800 shrink-0">
            <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center font-bold text-white text-sm shadow-sm transition-transform duration-300 ease-out hover:rotate-6 hover:scale-110">
                {{ Str::of(config('app.name'))->substr(0, 1) }}
            </div>
            <span class="font-semibold text-white tracking-tight truncate">{{ config('app.name') }}</span>
        </div>

        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            @unless (auth()->user()->isPlainEmployee())
                <a href="{{ route('dashboard') }}"
                   class="group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200
                          {{ request()->routeIs('dashboard') ? 'bg-emerald-600 text-white shadow-md shadow-emerald-900/30' : 'text-slate-300 hover:bg-slate-800 hover:text-white hover:translate-x-1' }}">
                    <svg class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $icons['home'] !!}</svg>
                    {{ __('Dashboard') }}
                </a>
            @endunless
            ...
            @foreach ($navItems as $item)
                <a href="{{ route($item['route']) }}"
                   class="group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200
                          {{ request()->routeIs($item['pattern']) ? 'bg-emerald-600 text-white shadow-md shadow-emerald-900/30' : 'text-slate-300 hover:bg-slate-800 hover:text-white hover:translate-x-1' }}">
                    <svg class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $icons[$item['icon']] !!}</svg>
                    {{ __($item['label']) }}
                </a>
            @endforeach
            ...
            <a href="{{ route('alerts.index') }}"
               ...
               class="group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200
                      {{ request()->routeIs('alerts.*') ? 'bg-emerald-600 text-white shadow-md shadow-emerald-900/30' : 'text-slate-300 hover:bg-slate-800 hover:text-white hover:translate-x-1' }}">
                <svg class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $icons['bell'] !!}</svg>
                <span class="flex-1">{{ __('Alertas') }}</span>
        ...
        <div class="border-t border-slate-800 p-3 shrink-0">
            <x-ui.dropdown align="top" width="56">
                <x-slot name="trigger">
                    <button class="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors duration-200 text-left">
